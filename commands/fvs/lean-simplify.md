---
name: fvs:lean-simplify
description: Simplify and golf verified Lean proofs while preserving compilation
...
<objective>
Orchestrate proof simplification for a verified Lean spec using three-phase subagent dispatch (baseline -> research -> iterative simplify). Dispatches fvs-researcher for 3-lens analysis, then iteratively dispatches fvs-lean-simplifier to apply tiered heuristics ONE CHANGE AT A TIME, verifying compilation after each.
...
Output: Simplified spec file with before/after metrics, or NO_CHANGE report if proofs are already clean.
</objective>

<execution_context>
@~/.claude/fv-skills/workflows/lean-simplify.md
...
3. If no override, look up profile table:
   - quality: fvs-researcher=inherit, fvs-lean-simplifier=inherit
   - balanced: fvs-researcher=sonnet, fvs-lean-simplifier=sonnet
   - budget: fvs-researcher=haiku, fvs-lean-simplifier=sonnet
...
```bash
LEAN_SIMPLIFICATION=$(cat ~/.claude/fv-skills/references/lean-simplification.md)
...
**If --report-only:** Display research findings and exit. No simplification passes.

## Step 6: Iterative Simplifier Dispatch
...
- **SIMPLIFIED:** At least one change applied and build passes
- **NO_CHANGE:** No simplifications possible at current mode's tier ceiling
- **ERROR:** Build failures that could not be reverted

## Step 8: Update CODEMAP.md If Exists

If .formalising/CODEMAP.md exists, verification status remains [OK] (simplification preserves verification -- zero sorry before and after).
...
```
>> Proof simplified. Consider committing the changes.

git add {spec_path}
git commit -m "refactor: simplify {function_name} proof"
...
Try --mode aggressive for more aggressive simplification.
/fvs:plan to select next verification target
```
