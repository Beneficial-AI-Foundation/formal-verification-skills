...
## A. Skill Invocation
- This skill is invoked by mentioning `$fvs:lean-verify`.
- Treat all user text after `$fvs:lean-verify` as `{{FVS_ARGS}}`.
...
<execution_context>
@${CLAUDE_PLUGIN_ROOT}/fv-skills/workflows/lean-verify.md
...
BASELINE_STYLE_DIAGNOSTICS=$(node ${CLAUDE_PLUGIN_ROOT}/scripts/fvs-lean-style-check.mjs \
